...
# # ── Configuració d'estoc (ha de coincidir amb mystock.py) ────────────────────

# class StockConfig:
#     """Configuració de les columnes del Google Sheet d'estoc."""
#     STOCK       = 0   # Quantitat
#     STORAGE     = 1   # Ubicació
...
# # ── Classe per consultar l'estoc ─────────────────────────────────────────────

# class StockChecker:
#     """
#     Carrega el Google Sheet d'estoc una vegada i permet cercar components.

#     Ús:
#         checker = StockChecker()
#         ok = checker.load()          # bloquejant, cridar des d'un thread
#         qty, loc = checker.find("C17902")
#     """

#     def __init__(self):
#         self._data: List[List[str]] = []   # files [0] = capçalera
#         self._loaded = False

#     # ── Càrrega ───────────────────────────────────────────────────────────────

#     def load(self) -> bool:
#         """Carrega les dades del Google Sheet. Retorna True si ha anat bé."""
#         try:
#             import gspread
...
#                 print(f"[StockChecker] Fitxer de credencials no trobat: {creds_path}")
#                 return False
...
#             if not data:
#                 print("[StockChecker] El Google Sheet d'estoc està buit.")
#                 return False

#             self._data   = data
#             self._loaded = True
#             print(f"[StockChecker] Carregades {len(data) - 1} files d'estoc.")
#             return True

#         except Exception as e:
#             print(f"[StockChecker] Error en carregar l'estoc: {type(e).__name__}: {e}")
#             return False

#     # ── Cerca ─────────────────────────────────────────────────────────────────

#     def find(self, *search_terms: str) -> Tuple[Optional[str], Optional[str]]:
#         """
#         Cerca un component per qualsevol dels termes indicats (codi, nom…).
#         La cerca és insensible a majúscules i comprova la columna REFERENCE.

#         Retorna (stock, storage) o (None, None) si no es troba.
#         """
#         if not self._loaded or len(self._data) < 2:
#             return None, None

#         # Normalitzar termes (lletra minúscula, sense espais al voltant)
#         terms = [t.strip().lower() for t in search_terms if t and t.strip()]
#         if not terms:
#             return None, None

#         for row in self._data[1:]:           # saltar capçalera
#             ref = row[StockConfig.REFERENCE].strip().lower()
#             if any(term == ref for term in terms):
#                 qty     = row[StockConfig.STOCK].strip()
#                 storage = row[StockConfig.STORAGE].strip()
#                 return qty or "—", storage or "—"

#         # Segona passada: cerca parcial (per si el ref és un substring)
...
#     error         = Signal(str)
#     stock_found   = Signal(str, str)        # qty, storage
...
# QLabel#label_fabricantvariable,
# QLabel#label_packagevariable,
# QLabel#label_stockvariable,
# QLabel#label_4 {
#     font-size: 10px;
#     color: #1a1a2e;
# }

# /* ── Valors d'estoc: verd si en tenim, vermell si no ───────────── */
# QLabel#label_stockvariable[stock_ok="true"]  { color: #1e7e34; font-weight: bold; }
# QLabel#label_stockvariable[stock_ok="false"] { color: #c62828; font-style: italic; }
...
#         # ── StockChecker: es carrega en background en iniciar l'app ──────────
#         self._stock_checker = StockChecker()
#         self._last_search_code = ""   # codi de la última cerca, per buscar a l'estoc
...
#         if ok:
#             print("[StockChecker] Estoc carregat correctament.")
#         else:
#             print("[StockChecker] No s'ha pogut carregar l'estoc.")

#     # ── Helpers d'estoc ───────────────────────────────────────────────────────

#     def _reset_stock_labels(self):
#         """Mostra '—' als labels d'estoc (estat inicial o no trobat)."""
#         self.ui.label_stockvariable.setText("—")
#         self.ui.label_stockvariable.setProperty("stock_ok", "")
#         self.ui.label_stockvariable.style().unpolish(self.ui.label_stockvariable)
#         self.ui.label_stockvariable.style().polish(self.ui.label_stockvariable)
#         self.ui.label_4.setText("—")

#     def _on_stock_found(self, qty: str, storage: str):
#         """Actualitza la UI quan el component es troba a l'estoc."""
#         self.ui.label_stockvariable.setText(qty)
#         self.ui.label_stockvariable.setProperty("stock_ok", "true")
#         self.ui.label_stockvariable.style().unpolish(self.ui.label_stockvariable)
#         self.ui.label_stockvariable.style().polish(self.ui.label_stockvariable)
#         self.ui.label_4.setText(storage)

#     def _on_stock_missing(self):
#         """Actualitza la UI quan el component NO es troba a l'estoc."""
#         self.ui.label_stockvariable.setText("No trobat")
#         self.ui.label_stockvariable.setProperty("stock_ok", "false")
#         self.ui.label_stockvariable.style().unpolish(self.ui.label_stockvariable)
#         self.ui.label_stockvariable.style().polish(self.ui.label_stockvariable)
#         self.ui.label_4.setText("—")
...
#         """
#         Cerca el component a l'estoc i emet el signal adequat.
#         S'ha de cridar des d'un thread secundari.
#         """
#         # Esperar que l'estoc acabi de carregar (màx 15 s)
#         import time
#         waited = 0
...
#             # Si l'estoc no s'ha pogut carregar, no mostrem res
#             return

#         # Termes de cerca: codi original + nom del component (per si MPN ≠ codi)
#         component_name = str(component.get("name", "")).strip()
#         qty, storage = self._stock_checker.find(search_code, component_name)

#         if qty is not None:
#             self._signals.stock_found.emit(qty, storage)
#         else:
...
#         # Cerca a l'estoc en background (no bloqueja la UI)
#         threading.Thread(

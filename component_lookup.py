...
try:
    ...
    logger.warning("No s'han pogut importar els mòduls de proveïdor: %s", e)
...
class StockChecker:
    ...
    def load(self) -> bool:
        try:
            ...
            creds_path = resource_path(Config.GOOGLE_CREDENTIALS_JSON)
            if not creds_path.exists():
                logger.warning("[StockChecker] Credencials no trobades: %s", creds_path)
                return False
            ...
            if not self._data:
                logger.warning("[StockChecker] Full d'estoc buit.")
                return False

            self._loaded = True
            logger.info("[StockChecker] %d files carregades.", len(self._data) - 1)
            return True

        except Exception as e:
            logger.error("[StockChecker] Error en carregar: %s: %s", type(e).__name__, e)
            return False
    ...
    def update_row(self, sheet_row: int, qty: str, storage: str) -> bool:
        if not self._loaded or self._worksheet is None:
            logger.warning("[StockChecker] No es pot escriure: estoc no carregat.")
            return False
        ...
        try:
            ...
            logger.info("[StockChecker] Fila %d actualitzada → qty=%r, storage=%r", sheet_row, qty, storage)
            return True

        except Exception as e:
            logger.error("[StockChecker] Error en escriure: %s: %s", type(e).__name__, e)
            return False
...
def is_out_of_stock(avail: str) -> bool:
    """Retorna True si la disponibilitat indica que no hi ha estoc a la botiga."""
    # IMP-006: eliminat print(avail) de debug
    if not avail:
        return False
    a = avail.strip().lower()
    return a in ("0", "out of stock", "no stock", "discontinued",
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # IMP-005 (pendent): QNetworkAccessManager instanciat una sola vegada
        self._nam = QNetworkAccessManager()
        self._nam.finished.connect(self._on_image_loaded)
    ...
    def _fix_result_layouts(self):
        ...
        for w in (self.ui.widget_imatge,):
            w.setSizePolicy(compact)
    ...
    def _load_stock_background(self):
        ...
        self._stock_loading = False
        if ok:
            logger.info("[StockChecker] Estoc carregat correctament.")
        else:
            logger.warning("[StockChecker] No s'ha pogut carregar l'estoc.")
    ...
    def _on_search(self):
        ...
        self._last_search_code = effective_code
    ...
    def _populate_result(self, component: dict, supplier: str):
        ...
        if img_url:
            # IMP-005: reutilitzem self._nam instanciat al constructor
            req = QNetworkRequest(QUrl(img_url))
            req.setRawHeader(
                QByteArray(b"User-Agent"),
                QByteArray(b"Mozilla/5.0 (Windows NT 10.0; Win64; x64) "
...
def main():
    # IMP-006: Logging configurat a WARNING per defecte.

...
def get_lcsc_data(part_number):
    ...
    logger.info("Cercant '%s' a LCSC", part_number)
    ...
    try:
        ...
        logger.error("Error de connexió LCSC: %s", e)
        return None
    
    try:
        data = response.json()
    except json.JSONDecodeError as e:
        logger.error("Error descodificant JSON de LCSC: %s", e)
        return None
    
    if data.get("code") != 200 or not data.get("result"):
        logger.warning("Resposta no vàlida de LCSC per a '%s' (code=%s)", part_number, data.get("code"))
        return None
    ...
    logger.info("Component LCSC trobat: %s (%s)", component["name"], part_number)
    return component


def search_by_mpn(mpn):
    """Cerca per MPN a LCSC i retorna el codi LCSC (Cxxxx)."""
    url = "https://wmsc.lcsc.com/ftps/wm/search/global"
    params = {
        "keyword": mpn,
        "currentPage": 1,
    ...
    try:
        ...
        logger.error("Error cercant MPN '%s' a LCSC: %s", mpn, e)
        return None
    ...
    if not result_list:
        logger.warning("Cap resultat per MPN '%s' a LCSC", mpn)
        return None

    product = result_list[0]
    logger.info("MPN '%s' trobat a LCSC: %s", mpn, product.get("productModel"))
    return product.get("productCode")
...
if __name__ == "__main__":
    ...
    components = get_lcsc_data("C364262")
    if components:
        print(">>>>>>>>>>>")
        for key, value in components.items():
            print(f"{key}: {value}")
...
try:
    from secrets import API_KEY_MOUSER
except ImportError:
    API_KEY_MOUSER = ""
    logger.warning(
        "secrets.py no trobat. Crea el fitxer secrets.py amb API_KEY_MOUSER. "
...
def get_mouser_data(part_number, api_key=API_KEY_MOUSER):
    ...
    logger.info("Cercant '%s' a Mouser", part_number)
    ...
    try:
        ...
        logger.error("Error de connexió Mouser: %s", e)
        return None
    
    try:
        data = response.json()
    except json.JSONDecodeError as e:
        logger.error("Error descodificant JSON de Mouser: %s", e)
        return None
    
    if "Errors" in data and data["Errors"]:
        for error in data["Errors"]:
            logger.error("Error de l'API Mouser: %s", error.get("Message", "Error desconegut"))
        return None
    ...
    if not parts:
        logger.warning("Cap component trobat a Mouser amb part number: %s", part_number)
        return None
    ...
    logger.info("Component trobat a Mouser: %s", component["name"])
    return component
...
if __name__ == "__main__":
    ...
    components = get_mouser_data("863-NGTB40N120FL2WG", API_KEY_MOUSER)
    if components:
        print(">>>>>>>>>>>")
        for key, value in components.items():
            print(f"{key}: {value}")
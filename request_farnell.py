...
# IMP-001: API key importada des de secrets.py (no al codi font)
try:
    from secrets import API_KEY_FARNELL
except ImportError:
    API_KEY_FARNELL = ""
    logger.warning(
        "secrets.py no trobat. Crea el fitxer secrets.py amb API_KEY_FARNELL. "
        "Consulta secrets.py.example per al format."
    )
...
def get_farnell_data(part_number, api_key, store_region="es.farnell.com"):
    ...
    logger.info("Cercant %s a Farnell (%s)", part_number, store_region)
    ...
    try:
        ...
        logger.error("Error de connexió Farnell: %s", e)
        return None
    
    try:
        data = response.json()
    except json.JSONDecodeError as e:
        logger.error("Error descodificant JSON de Farnell: %s", e)
        return None
    ...
    if "manufacturerPartNumberSearchReturn" in data:
        ...
        logger.warning("Resposta inesperada de l'API Farnell per a '%s'", part_number)
        return None
    
    products = search_return.get("products", [])
    
    if not products:
        logger.warning("Cap component trobat a Farnell amb part number: %s", part_number)
        return None
    ...
    logger.info("Component trobat a Farnell: %s", component["name"])
    return component
...
def get_farnell_by_sku(sku, api_key=API_KEY_FARNELL, store_region="es.farnell.com"):
    ...
    try:
        ...
        if "manufacturerPartNumberSearchReturn" in data:
            ...
            logger.warning("Resposta inesperada de l'API Farnell per al SKU '%s'", sku)
            return None
        
        products = search_return.get("products", [])
        
        if not products:
            logger.warning("Cap component trobat a Farnell amb SKU: %s", sku)
            return None
        ...
        if not product:
            logger.warning(
                "No s'ha trobat coincidència exacta per al SKU '%s' (%d resultats similars)",
                sku, len(products)
            )
            return None
        ...
        logger.info("Component Farnell SKU '%s' trobat: %s", sku, component["name"])
        return component
        
    except Exception as e:
        logger.error("Error cercant SKU Farnell '%s': %s", sku, e)
        return None
...
def debug_product_fields(part_number, api_key, store_region="es.farnell.com"):
    """
    Funció de debug per veure tots els camps disponibles d'un producte.
    Ús: cridar des de __main__ per inspecció interactiva.
    """
    component = get_farnell_data(part_number, api_key, store_region)
...
def search_farnell_keyword(keyword, api_key, store_region="es.farnell.com", max_results=10):
    """
    Cerca per paraula clau. Ús: cridar des de __main__ per explorar components.
    """
    url = "https://api.element14.com/catalog/products"
...
if __name__ == "__main__":
    ...
    print("CERCA PER FARNELL SKU/ORDER CODE")
    print("="*80 + "\n")
    component2 = get_farnell_by_sku("2988437", API_KEY_FARNELL, STORE_REGION)
    if component2:
        for key, value in component2.items():
            print(f"{key}: {value}")
        print()